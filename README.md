...
- 연락처 표시: 운영팀 결정(26-08-14)으로 캠퍼스 탭은 이메일/전화/학번 원본 표시 (`campus-apps-script.gs`의 `INCLUDE_CONTACTS = true`).
  ⚠️ 웹앱 URL을 알면 비밀번호 게이트 없이 데이터 접근 가능 — 민감도가 올라가면 `false`로 되돌리고 새 버전 배포.
  폴백 스냅샷(`campus-snapshot.json`)은 공개 레포에 커밋되므로 **마스킹본만** 커밋할 것.
- 표/셀 복사: [⧉ 표 복사] 버튼 = 표 전체를 TSV로 (시트에 바로 붙여넣기), 셀·항목 클릭 = 그 값 복사
...
GitHub Pages 공개 배포에 맞춰 **사회혁신가·멘토 탭은 연락처(전화·이메일)를 싣지 않는다.**
`apps-script.gs`의 `INCLUDE_CONTACTS = false` 상태를 유지할 것 — 연락처가 필요하면 시트에서 직접 확인.
(캠퍼스 탭만 운영팀 결정으로 예외 — 위 '캠퍼스 탭' 절 참고.)
